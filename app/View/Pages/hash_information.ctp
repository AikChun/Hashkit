...
<!--                 <ul class="nav hidden-xs" id="lg-menu">
						...
						<div class="column col-sm-10 col-xs-11" id="main">
							 ...
							 <div class=" col-sm-10 col-xs-11">
											...
											<hr >
												<h3 >What about Cryptographic Hash function ?</h3>
											...
											<p>
											Broken - Better than brute force attack is available. Known internal weakness in hash function structure.
											</p>
											<hr>

											<h3 id="compuinf">How can the hash function to be considered to have computational in-feasibility?</h3>
											<br>
											<p>
											In mathematical sense, the computational in-feasibility is directly related to complexity theory which means that no polynomial complexity algorithm can solve the given problem or a attack is simply infeasible to be conducted due to the computational restrictions of available hardware. 
											</p>
											<hr>
											...
											<p>
											For a function to be categorized as a one way hash functions, it has to fulfill the two security properties: <a href="#preimageresist">pre-image resistance</a> and <a href="#secpreimageresist">second pre-image resistance</a>
											</p>
											<br>
											<h4><b>Collision Resistant Hash Functions (CRHF)</b></h4>
											<p>
											For a function to be categorized as Collision Resistant hash functions, it has to fulfill the two security properties: <a href="#preimageresist">pre-image resistance</a> and <a href="#collisionresistance">collision resistance</a>
											</p>
											<hr >
											...
											<br>
											SHA stands for Secure hash algorithm. When it first came out in the public in 1993, SHA0 algorithm was designed by National Security Agency and was made a standard for applications. it generates the 160-bit hash value and uses the Merkle–Damgård construction, which is similar to MD5.
											<br>
											...
											<p class="edit_area">This is often used to check the integrity of the data by comparing the both checksums of two identical files generated by same hash function to be similar to one another.</p>
											Example :
											<br>
											<CENTER><img src="/img/fileIntegrity.jpg" class="img-responsive"></CENTER>
											<br>
											<h4 id="hashingpasswords"><b>Hashing passwords</b></h4>
											<p class="edit_area">In a computer system, it is not a good idea to store passwords in clear-text (readable to user) due to the fact that if the system has been compromised, the bad guy can somehow obtain the stored passwords and use to their advantage. Hence, a more secure way is to store the hash of the password, rather than the password itself since the hashes are not reversible. This reduces the consequence of damage done by the bad guy.</p> 
											Example : 
											<br>
											...
											<br><br> 
											<p >A cryptographic hash functions must be able to defend against all known type of cryptanalytic attack. It should be able to apply to any input size and output a fixed size. It must have the following properties:
													<br><br>
													...
													<br>
													<h4><b>Some facts about these properties</b></h4>
													<br>
											...
											<br>
											Every cryptographic hash algorithm should exhibit a certain degree of avalanche effect and if it does not, it shows a poor randomization. And when it produce poor randomization, cryptanalyst can use it to make predications which is useful to completely or sufficient to partially break algorithm. To see if the algorithm you chose has a degree of avalanche effect, you can choose this tool: <a href="/HashTests/avalanche_effect">link</a> 
											<br>
											...
											<hr>
												...
												<p>An attack where the adversary only has access to message digest and is trying to generate a plain text that hash to the same value. This attack usually occur when the database of hashed password is leaked or hacked.
												</p>
												<br>
												<h4><b>2nd pre-image attack</b></h4>
												<p>An attack where the adversary only has access to the input plain-text and is trying to find another plain-text that hash to the same value. It is important for hash function to be resistant to 2nd pre-image attacks because of digital signature schemes where its H(message) and message is made public. An attacker can come up with another H(message) using different plain-text and publish the message as though he is the original signer.</p>
												<br>
												<br>
												...
												<br>
												In recent years, this method of cryptanalysis is usually done on symmetric ciphers but it is also applicable in the case of cryptographic hash functions like MD5, SHA-0 and etc. This method is all about finding a correlation between the differences (XOR or modular) in input and output of a hash function with difference in output being equal to zero to produce a full collision or differ slightly to obtain so called near-collision.
												<br>
												...
												<br>
												This method is to approximate the internal states of the cryptographic hash function with its origin from cipher analysis. An example of a linear attack is when cryptanalyst found out solutions of a system of non linear equations describing internal states of the LASH hash function.
												<br>
												<h4><b>random graph theory attack</b></h4>
												<br>
												Collision Search methods such p-Pollard method are based on the structure of a random mapping directed graph (digraph) and from that, finding two distinct points in the same connected component on the function digraph such that there is no directed path between both of them.
												<br>
												...
												<br>
												<a href="http://www.cl.cam.ac.uk/~rja14/Papers/hash.pdf">Classification of Hash functions</a>
												<br>
